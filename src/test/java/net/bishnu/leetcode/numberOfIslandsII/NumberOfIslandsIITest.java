...
public class NumberOfIslandsIITest {
    ...
    public void tesT() throws IOException {
        ...
        for(int index=0; index<positions.length; index++){
            ...
            int col = positions[index][1];
            map[row][col] = 1;
            try(FileOutputStream fos = new FileOutputStream(index+".txt")){
                String position = "row:"+row+","+" col:"+ col+System.lineSeparator();
                fos.write(position.getBytes());
                for(int i=0; i<map.length; i++){
                    StringBuilder sb = new StringBuilder();
                    for(int j=0; j<map[i].length; j++){
                        if(i==row && j==col) sb.append("-");
